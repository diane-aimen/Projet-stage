...
class profController extends Controller
{
    ...
    public function editProf($id)
    {
        ...
        if(!$prof)
           redirect() -> route('show.all.prof') -> with(['Erreur' => "Enseignant n'existe pas !!!"]); // maybe need else
    ...
    try {
        Enseignant::where('id_enseignant',$request ->id) -> update(
            [
                'nom_ens' => $request->nom,
...
{
    $prof = Enseignant::findOrFail($id);
    $prof->matieres()->delete();

    $prof-> delete();      // maybe wrong
